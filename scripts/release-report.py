...
def main() -> None:
    lock = json.loads(
        (Path(__file__).resolve().parent.parent / "conformance" / "fixtures.lock.json")
        .read_text()
    )
    ...
    report = {
        ...
        "benchmarks_same_run": benchmarks,
        "gates": {
            "G1_conformance": "not run (no pinned corpus)",
            "G2_zero_intermediates": all(
                b is not None for b in [True]
            ),
            ...
            "G5_codec_floor": "not run (competing compiled codecs not installed here)",
